        ...
        var handleDataTableFixedHeader = function() {
            ...
            if ($('#data-table-fixed-header').length !== 0) {
                var table=$('#data-table-fixed-header').DataTable({
                    ...
                    responsive: true,
                    ajax:"{{ url('tugas/get_data_siswa')}}?kelas={{$data->kelas}}&kelas_room_id={{$data->id}}&kat=1",
                      columns: [
                    ...
                    <table id="data-table-fixed-header" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
                      <thead>
                        <tr role="row">
                          ...
                          <th class="sorting_asc" width="8%">NIS</th>
                          <th class="sorting" >Nama</th>
                          <th class="sorting" width="15%" >Kelas</th>
                          <th class="sorting" width="15%" >Jurusan</th>
                          <th class="sorting" width="3%" >Cek</th>
                          <th class="sorting" width="9%" >Nil PG</th>
                          <th class="sorting" width="9%" >Nil Esay</th>
                          <th class="sorting" width="9%" >Nilai</th>
                        </tr>
                      </thead>
...
<script> 
      ...
      function tambah(id){
        ...
        $('#form-tambah').load("{{url('master/siswa/modal')}}?id="+id);
      }
      ...
      function delete_data(id){
           ...
           swal({
               ...
               if (willDelete) {
                       ...
                       $.ajax({
                           type: 'GET',
                           url: "{{url('master/siswa/delete_data')}}",
                           data: "id="+id,
                           success: function(msg){
                               ...
                               var tables=$('#data-table-fixed-header').DataTable();
                                  tables.ajax.url("{{ url('master/siswa/get_data')}}").load();
                           }
                       });
      ...
      function proses_user(id){
           ...
           swal({
               title: "Yakin memproses akses login untuk semua siswa ?",
               text: "",
               type: "warning",
               icon: "error",
               showCancelButton: true,
               ...
               if (willDelete) {
                       ...
                       $.ajax({
                           type: 'GET',
                           url: "{{url('master/siswa/proses_user')}}",
                           data: "id="+id,
                           success: function(msg){
